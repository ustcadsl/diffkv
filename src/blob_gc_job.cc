...
BlobGCJob::~BlobGCJob() {
  ...
  // metrics_.gc_bytes_relocated); RecordTick(stats_, BLOB_DB_GC_NUM_NEW_FILES,
  // metrics_.gc_num_new_files); RecordTick(stats_, BLOB_DB_GC_NUM_FILES,
  // metrics_.gc_num_files); RecordTick(stats_, TitanStats::GC_DISCARDABLE,
  // metrics_.gc_discardable); RecordTick(stats_, TitanStats::GC_SMALL_FILE,
  // metrics_.gc_small_file); RecordTick(stats_, TitanStats::GC_SAMPLE,
  // metrics_.gc_sample);
}
